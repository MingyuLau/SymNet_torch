...
class CZSL_Evaluator:
    ...
    def __init__(self, dset, model, device):
        ...
        self.pairs = torch.LongTensor(pairs).to(self.device)
        ...
        self.closed_mask = torch.ByteTensor(mask).to(self.device)
        ...
        for _obj in dset.objs:
            ...
            oracle_obj_mask.append(torch.ByteTensor(mask).to(self.device))
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, histogram=False, synonym_mode=False):
        ...
        closed_1_match = (attr_truth == predictions['closed1'][0]).float() * (
                    obj_truth == predictions['closed1'][1]).float()
        closed_2_match = (attr_truth == predictions['closed2'][0]).float() * (
                    obj_truth == predictions['closed2'][1]).float() + closed_1_match
        closed_3_match = (attr_truth == predictions['closed3'][0]).float() * (
                    obj_truth == predictions['closed3'][1]).float() + closed_2_match
...
class GCZSL_Evaluator:
    ...
    def __init__(self, dset):
        ...
        # [('sliced', 'apple'), ('ripe', 'apple'), ...] --> torch.LongTensor([[0,1],[1,1], ...])
        pairs = [(dset.attr2idx[attr], dset.obj2idx[obj])
                 for attr, obj in dset.pairs]
